<?php
    ...
    class Regime_model extends CI_Model{
        ...
        public function getRegimeFit($gain, $height, $weight, $gender){
            $ans = array();
            if($gain){
                ...
                foreach ($result->result_array() as $row) {
                    if ($gender==1) {
                        $duree = round(10/((($height - 120)/ $weight) * $row['coefficient']));
                    }
                    else $duree = round(10/((($height - 100)/ $weight) * $row['coefficient']));
                    $values = array(
                ...
                foreach ($result->result_array() as $row) {
                    if ($gender==1) {
                        $duree = round(10/((($height - 80)/ $weight) * $row['coefficient']));
                    }
