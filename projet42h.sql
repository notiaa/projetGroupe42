...
ALTER TABLE account
  ADD CONSTRAINT `user_account_idfk` FOREIGN KEY (`idUser`) REFERENCES `user` (`idUser`);
...
CREATE TABLE transaction(
  ...
  -- si date transac null, l entre est en attente de validation
)ENGINE=InnoDB;
...
CREATE TABLE credit_attente(
  idAccount INT(11),
  code VARCHAR(10),
  UNIQUE(idAccount,code)
)ENGINE=InnoDB;
...
ALTER TABLE credit
  ADD CONSTRAINT `account_credit_fk` FOREIGN KEY (`idAccount`) REFERENCES `account` (`idAccount`);

CREATE TABLE key_char(
  key_value VARCHAR(3)
)ENGINE=InnoDB;

CREATE TABLE code_credit(
  numeric_value INT(11) AUTO_INCREMENT,
  key_char VARCHAR(3),
  used TINYINT(2) DEFAULT 0,
  value DECIMAL(10,0),
  UNIQUE(numeric_value,key_char)
)ENGINE=InnoDB;

 INSERT INTO code_credit VALUES(null, (SELECT key_value FROM key_char), 0, 15000);

 SELECT concat(key_value,'0000',numeric_value) from code_credit, key_char;
